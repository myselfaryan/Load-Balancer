...
import PriorityWeightedRoundRobin from './algorithms/pwrr.js';
import { printDataPWRR } from './data/data.js';
import Task from './Tasks/task.js';
import VirtualMachinePWRR from './VMs/pwrrVM.js';
...
async function run() {
	try {
		...
		for (let i = 1; i < vmLines.length; ++i) {
			if (!vmLines[i]) continue;
			const line = vmLines[i].split(',');
			if (line.length !== 2) {
				console.error(`Invalid VM line: ${vmLines[i]}`);
				continue;
			}
			vms.push(new VirtualMachinePWRR(+line[0], +line[1]));
		}
...
function processData() {
	...
	const loadBalancer = new PriorityWeightedRoundRobin(vms);
	...
	while (1) {
		let stat = true;
		for (let i of vms) {
			if (i.currentLoad != 0 || loadBalancer.pendingTasks.length != 0) {
				stat = false;
				break;
			}
		}

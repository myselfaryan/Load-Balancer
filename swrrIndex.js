...
import StaticWRR from './algorithms/staticWRR.js';
import { printDataSWRR } from './data/data.js';
import Task from './Tasks/task.js';
import VirtualMachineSWRR from './VMs/staticWrrVM.js';
...
async function run() {
	try {
		...
		for (let i = 1; i < vmLines.length; ++i) {
			...
			vms.push(new VirtualMachineSWRR(+line[0], +line[1]));
		}
...
function processData() {
	...
	const loadBalancer = new StaticWRR(vms);
